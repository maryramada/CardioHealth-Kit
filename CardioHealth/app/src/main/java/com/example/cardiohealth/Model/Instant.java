...
import static com.example.cardiohealth.Model.Data.stringToData;
...
public class Instant implements Comparable<Instant> {
    ...
    public int compareTo(Instant other) {
        // Comparar a data usando o compareTo da classe Data
        int dateComparison = this.data.compareTo(other.data);
        if (dateComparison != 0) {
            return dateComparison;
        }

        // Se as datas forem iguais, comparar pelo tempo (hora, minuto, segundo)
        int timeComparison = Integer.compare(this.hora, other.hora);
        if (timeComparison != 0) {
            return timeComparison;
        }

        timeComparison = Integer.compare(this.minuto, other.minuto);
        if (timeComparison != 0) {
            return timeComparison;
        }

        return Integer.compare(this.segundo, other.segundo);
    }
